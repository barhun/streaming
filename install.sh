...
[ -z "$IS_ARM" ] && {
  WIDEVINE_URL="https://dl.google.com/widevine-cdm/$(wget -qO - https://dl.google.com/widevine-cdm/versions.txt 2>/dev/null | tail -n 1)-linux-ia32.zip"
  ...
  WIDEVINE_URL="https://pi.vpetkov.net"

  for file in "manifest.json" "libwidevinecdm.so"; do
    wget -qO "$TMP_DIR/$file" "$WIDEVINE_URL/$file" 2>/dev/null || {
      rm -rf "$TMP_DIR"
      exit_with_error "Error occurred while downloading the Widevine library. Check your internet connection and try again."
    }
  done
...
sudo mv /etc/xdg/autostart/xcompmgr.desktop /root/backup.xcompmgr.desktop >/dev/null 2>&1
